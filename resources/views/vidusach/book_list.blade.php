    ...
    <table id = "book-table" class="table table-striped table-bordered" width="100%">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$row->gia_ban}}</td>
                <td><img src="{{asset('book_image/'.$row->file_anh_bia)}}" width="50px"></td>
                <td>
                    <div class="btn-group">
                        ...
                        <form method='post' action = "{{route('bookdelete')}}" onsubmit="return confirm('Bạn có chắc chắn muốn xóa cuốn sách này không?');">
                            ...
                            <for mmethod='post'action= "{{route('bookdelete')}}"
                                onsubmit="returnconfirm('Bạn có chắc chắn muốn xóa cuốn sách này không?');">
                                <input type='hidden'value='{{$row->id}}'name='id'>
